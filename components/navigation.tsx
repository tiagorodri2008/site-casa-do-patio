...
import {ArrowUpRight,CalendarDays,Menu,Moon,Sun,Utensils,X} from 'lucide-react';
...
export function Header({locale,slug=''}:{locale:Locale;slug?:string}) {
  ...
  const contacts=locale==='pt'?'Contactos':'Contact';
  ...
  return <>
    ...
    <header className={`header${hidden?' header-hidden':''}`}>
      ...
      <nav aria-label={locale==='pt'?'Navegação principal':'Main navigation'}>
        ...
        <a href={`${home}#ambiente`}>{copy.navGallery[locale]}</a>
        <a href={`${home}#contacto`}>{contacts}</a>
      </nav>
...
export function MobileActions({locale,slug=''}:{locale:Locale;slug?:string}) {
  ...
  return <nav className="mobile-actions" aria-label={locale==='pt'?'Ações rápidas':'Quick actions'}><a href={route(locale,'menu')}><Utensils size={17}/>{copy.menu[locale]}</a><a className="mobile-primary" href={route(locale,'reservas')}><CalendarDays size={17}/>{copy.book[locale]}</a></nav>;
}
